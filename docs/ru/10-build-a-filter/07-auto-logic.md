...
Так это выглядит вживую. Порог `150`, индекс дошёл до него — вентилятор включился сам:

![Автоматика включила вентилятор на пороге](../../img/10-filter/07-portal-auto-on.png)
*Режим `auto`: индекс `151` при пороге `150` — вентилятор включён.*

![То же в мобильном приложении](../../img/10-filter/07-app-auto-on.png)
*Приложение показывает то же состояние: значение выросло, вентилятор работает.*

Ручной режим перебивает автоматику: `Mode` → `on`, и вентилятор крутит, даже когда воздух уже чистый:

![Ручной режим: вентилятор включён при чистом воздухе](../../img/10-filter/07-portal-mode-on.png)
*Режим `on`, индекс `132` — ниже порога, но вентилятор работает: команда с портала важнее автоматики.*
