...
注意这些单元格：门户画的是图标和数值，不画标签。实体的`label`会用作屏幕阅读器的名称以及其他界面的名称，卡片上的空间因此省了下来。控件的标签则是可见的 —— 上图中的`Mode`和`VOC threshold`。
...
烧录后打开**门户的仪表板** —— 卡片就在出厂设备卡片所在的地方，以及移动应用中。设备自己的页面上没有它：那里是字典量的图表、菜单和集成。

![门户仪表板上的过滤器卡片](../../img/10-filter/06-portal-card.png)
*卡片按清单组装：VOC指数和风机在同一行，模式和阈值在下面。*

![移动应用中的同一张卡片](../../img/10-filter/06-app-card.png)
*应用从同一份清单构建卡片——不需要为手机写单独的代码。*

- 指数单元格显示实时数值（对传感器吹气——数字在下次更新时增长）；
- 风机单元格 —— 打开/关闭；
