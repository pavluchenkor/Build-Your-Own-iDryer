...
!!! warning "Não edite ficheiros gerados"
    ...
    O nome da constante de um item forma-se de modo simples: `MENU_` mais o seu `id` em maiúsculas. O item `target_temp` dá `MENU_TARGET_TEMP`, `hysteresis` — `MENU_HYSTERESIS`. Estas constantes fazem falta no capítulo 7.
...
O hook encontrará automaticamente o gerador no caminho `lib/idryer-core/menu/menu_gen.py`, portanto a biblioteca deve estar conectada através de `lib/` (symlink ou cópia), como descrito no capítulo 4. O gerador é executado pelo PlatformIO com o seu próprio Python - não é preciso instalar nada à parte. Se ainda assim a compilação falhar neste passo, mostre o texto do erro na comunidade: [Telegram](https://t.me/iDryer), [Discord](https://discord.gg/jGce5eeHHz).
...
![Menu do dispositivo no portal](../../img/09-cabinet/06-portal-menu.png)
*O menu chegou do dispositivo: temperatura de armazenamento e histerese com os seus limites. O valor pode ser alterado aqui mesmo - o dispositivo aceita-o, guarda-o e publica o menu de novo.*
